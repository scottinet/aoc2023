...
function convertSpelledNumberToNumber(str: string): string {
  ...
  for (const [index, stringifiedDigit] of STRINGIFIED_DIGITS.entries()) {
    // The 1st and last char of the stringified digit can be shared with another
    // stringified digit (e.g. "oneight" -> "18")
    // So, simplest way: always add the 1st and last char of the stringified
    // digit to the converted string
    // e.g. "oneight" -> "o1e8t"
